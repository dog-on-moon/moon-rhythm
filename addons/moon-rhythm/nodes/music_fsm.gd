...
func _ready() -> void:
	...
	var best_state: MusicState = null
	var best_state_beat := -1.0
	var best_hit: Hit = null
	
	for state: MusicState in _states:
...
var current_state: MusicState = null

func _on_state_cue(h: Hit, state: MusicState):
