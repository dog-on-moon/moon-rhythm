...
extends Node2D
class_name MusicFSM2D

const DEBUG_PRINT := false

var _states: Array[MusicState2D] = []
...
func _ready() -> void:
	...
	var best_state: MusicState2D = null
	var best_state_beat := -1.0
	var best_hit: Hit = null
	
	for state: MusicState2D in _states:
...
signal state_changed(state: MusicState2D)
var current_state: MusicState2D = null

func _on_state_cue(h: Hit, state: MusicState2D):
